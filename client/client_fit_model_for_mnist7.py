...
class learning_fit(object):
		...
		if gpus:
			try:
				...
				for gpu in gpus:
					...
					tf.config.per_process_gpu_memory_fraction = 0.3
	...
	def generate_datasets(self):		
		...
		sample_hete_list = [0, 1, 4]
		if self.random_choice_list == list():
			#self.random_choice_list = list(set([random.randint(10, 59999) for r in range(random.choice(list(set([random.randint(30, 39999) for r in range(30)]))))]))
			#self.random_choice_list = list(set([random.randint(36000, 41999) for r in range(10000)]))
			#self.random_choice_list = list(set([random.randint(0, 59999) for r in range(60000)]))
			self.random_choice_list = list(range(36000, 41999))
		'''	
		with open('./save_random_data/random_choice7_newloss.pickle', 'wb') as fw:
			pickle.dump(self.random_choice_list, fw)
		with open('./save_random_data/hete_list7_newloss.pickle', 'wb') as fw:
			pickle.dump(sample_hete_list, fw)
		
		with open('./save_random_data/random_choice7.pickle', 'rb') as fr:
			self.random_choice_list = pickle.load(fr)
		with open('./save_random_data/hete_list7.pickle', 'rb') as fr:
		...
		# new data --> class
		self.random_choice_list = list()
		sample_hete_list = [2, 3, 4, 5, 6, 7, 8, 9]
		if self.random_choice_list == list():
			self.random_choice_list = list(range(29000, 38999))
	...
	def manage_train(self, params=None, cr=None, cn=None, state=None): # cr:current_round
		...
		with open('./saved_weight/weights7.pickle', 'wb') as fw:
