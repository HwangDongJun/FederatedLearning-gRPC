...
class learning_fit(object):
	...
	def generate_datasets(self):		
		...
		sample_hete_list = [1, 2, 3, 6, 8]
		if self.random_choice_list == list():
			#self.random_choice_list = list(set([random.randint(10, 59999) for r in range(random.choice(list(set([random.randint(100, 49999) for r in range(30)]))))]))
			#self.random_choice_list = list(set([random.randint(6000, 11999) for r in range(1000)]))
			self.random_choice_list = list(range(6000, 11999))
		'''	
		with open('./save_random_data/random_choice2_newloss.pickle', 'wb') as fw:
			pickle.dump(self.random_choice_list, fw)
		with open('./save_random_data/hete_list2_newloss.pickle', 'wb') as fw:
			pickle.dump(sample_hete_list, fw)
		
		with open('./save_random_data/random_choice2.pickle', 'rb') as fr:
			self.random_choice_list = pickle.load(fr)
		with open('./save_random_data/hete_list2.pickle', 'rb') as fr:
		...
		# new data --> class
		self.random_choice_list = list()
		sample_hete_list = [0, 1, 4, 5, 6, 7, 9]
		if self.random_choice_list == list():
			self.random_choice_list = list(range(0, 11999))
	...
	def train_model_tosave(self, params, rounds, cn, breakin=False):
		...
		if params != None:
			...
			###
			print(f"### params==None {self.params[-2:]} ###")
	...
	def manage_train(self, params=None, cr=None, cn=None, state=None): # cr:current_round
		...
		with open('./saved_weight/weights2.pickle', 'wb') as fw:
