...
class learning_fit(object):
	...
	def generate_datasets(self):		
		...
		if self.random_choice_list == list():
			...
			self.random_choice_list = list(set([random.randint(0, 5999) for r in range(10000)]))

		with open('./save_random_data/random_choice11.pickle', 'wb') as fw:
			pickle.dump(self.random_choice_list, fw)
		with open('./save_random_data/hete_list11.pickle', 'wb') as fw:
			pickle.dump(sample_hete_list, fw)
		'''
		with open('./save_random_data/random_choice11.pickle', 'rb') as fr:
			self.random_choice_list = pickle.load(fr)
		with open('./save_random_data/hete_list11.pickle', 'rb') as fr:
	...
	def manage_train(self, params=None, cr=None, cn=None): # cr:current_round
		...
		with open('./saved_weight/weights11.pickle', 'wb') as fw:
