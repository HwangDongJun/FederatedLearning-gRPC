...
class learning_fit(object):
	...
	def generate_datasets(self):		
		...
		if self.random_choice_list == list():
			#self.random_choice_list = list(set([random.randint(10, 59999) for r in range(random.choice(list(set([random.randint(1000, 39999) for r in range(30)]))))]))
			self.random_choice_list = list(set([random.randint(15000, 19999) for r in range(10000)]))
			
		with open('./save_random_data/cifar/random_choice4.pickle', 'wb') as fw:
			pickle.dump(self.random_choice_list, fw)
		with open('./save_random_data/cifar/hete_list4.pickle', 'wb') as fw:
			pickle.dump(sample_hete_list, fw)
		'''
		with open('./save_random_data/cifar/random_choice4.pickle', 'rb') as fr:
			self.random_choice_list = pickle.load(fr)
		with open('./save_random_data/cifar/hete_list4.pickle', 'rb') as fr:
	...
	def manage_train(self, params=None, cr=None, cn=None): # cr:current_round
		...
		with open('./saved_weight/weights4.pickle', 'wb') as fw:
