	...
	for req in request:
		if req.ready_req.type == 'R':
			...
			if cur_learning.fetchone()[0] != 0:
				...
				cur_learning.execute('''INSERT INTO NowStatus (status_on, status_off) VALUES (?, ?);''', (1, 0,))

			conn_index.commit()

			# api
			r = requests.get('http://192.168.1.119:5000/new_client')
			...
			# api
			r = requests.get('http://192.168.1.119:5000/train_done')
...
def start_dashboard_server():
	#os.system("node ./FLDashboard/index.js")
	os.system("node ./dashboard/index.js")

if __name__ == '__main__':
	proc = mp.Process(name="subprocess", target=start_dashboard_server)
	proc.start()
